...
class DBmySQL:
    """MySQL backend implementation for department and employee management."""
    ...
    def createTables(cursor):
        """Create the required Department and Employee tables in MySQL."""
        DBmySQL.createTable(cursor, 1)
        DBmySQL.createTable(cursor, 2)

    def createTable(cursor, tableName):
        """Create a MySQL table by name identifier: 1 for Department, 2 for Employee."""
        if tableName == 1:
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS Department (
                    cod_d INT PRIMARY KEY AUTO_INCREMENT,
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS Employee (
                    ...
                    salary INT NOT NULL,
                    dprtmnt INT NULL,
                    FOREIGN KEY (dprtmnt) REFERENCES Department(cod_d)
                )
            """)
        else:
            print("Incorrect table name.")
    def commitChanges(conn):
        """Commit pending MySQL transaction changes."""
        conn.commit()
    ...
    def searchEmpl(cursor,dni):
        ...
        cursor.execute("SELECT dni FROM Employee WHERE dni = %s", (dni,))
    ...
    def searchDepByName(cursor, depName):
        """Search the Department table by department name and print the result if found."""
        cursor.execute("SELECT cod_d, depName, depDirdni FROM Department WHERE depName = %s", (depName,))
        result = cursor.fetchone() 
        if result:
            DBmySQL.showDepartmentSQL(cursor, result)
        else:
            print("Couldn't find a department with this name.")
        return result if result else None 
    
    def searchDepByCode(cursor):
        """Prompt for a department code, search it in MySQL, and display the result."""
        ...
        try:
            ...
            return None
        cursor.execute("SELECT * FROM Department WHERE cod_d = %s", (code,))
        result = cursor.fetchone()
        if result:
            DBmySQL.showDepartmentSQL(cursor, result)
            return result
    ...
    def insertNewDep(cursor):
        """Prompt for department data and insert a new department into MySQL."""
        depName = input("Please enter the name of the Department: ")
        foundDep=DBmySQL.searchDepByName(cursor,depName)
        if foundDep:
            ...
            if depDirdni:
                ...
                if depDirdni.lower() == 'null':
                    ...
                    found = DBmySQL.searchEmpl(cursor, depDirdni)
                    if not found:
            ...
            if doubleCheck == 'y':
                ...
                cursor.execute("INSERT INTO Department (cod_d, depName, depDirdni) VALUES \
                               (%s, %s, %s)", (newDep.depCode(), newDep.depName, newDep.depDirdni))
    
    
    def insertNewEmpl(cursor):
        """Prompt for employee information and insert a new employee record into MySQL."""
        emplDNI=input("Please enter the employee's DNI: ")
        foundEmpl = DBmySQL.searchEmpl(cursor,emplDNI)
        if foundEmpl == None:
            ...
            while foundDep == None:
                ...
                foundDep = DBmySQL.searchDepByName(cursor,depPertenece)       #check if it exists
                if foundDep == None:                               
            ...
            cursor.execute("INSERT INTO Employee (dni, empName, salary, dprtmnt) \
                           VALUES (%s,%s,%s,%s)",(newEmpl.empDni(),newEmpl.empName,newEmpl.salary,newEmpl.depPert))
            print("Successfully inserted a new employee! ")
                
    def showAllDep(cursor):
        """Retrieve and print all departments from MySQL."""
        ...
        for each in results:
            ...
            DBmySQL.showDepartmentSQL(cursor, newDep)
    ...
    def showAllEmpl(cursor):
        """Retrieve and print all employees from MySQL."""
        ...
        for each in result:
            ...
            DBmySQL.showEmployee(cursor, each)
        return listaEmpleadosAsObjects

    def showEmplByDep(cursor,depName):
        """Retrieve and print employees in a given department."""
        # include department name as the 4th column so showEmployee can display it
        cursor.execute("SELECT e.dni, e.empName, e.salary, d.depName FROM Employee e JOIN Department d ON e.dprtmnt = d.cod_d WHERE d.depName = %s", (depName,))
        result = cursor.fetchall()
        if result:
            print(f"Employees in department {depName}:")
            for emp in result:
                DBmySQL.showEmployee(cursor, emp)
        else:
            print(f"No employees found in department {depName}.")
        return result

    def updateDep(cursor):
        """Prompt the user to update a department's name or director DNI."""
        ...
        if foundDep:
            if choice == 1:     #update department's name
                ...
                cursor.execute("UPDATE Department SET depName = %s WHERE cod_d = %s", (newName, foundDep[0]))
                ...
                foundDNI = DBmySQL.searchEmpl(cursor,newDirDni)
                if not foundDNI:
                    input(f"No employee has been found with dni: {newDirDni}")
                    newDirDni = None  
                cursor.execute("UPDATE Department SET depDirdni = %s WHERE cod_d = %s", (newDirDni, foundDep[0]))
                print("Department director's DNI updated successfully!")
        else:
            print("Department not found. Please try again.")

    def updateEmpl(cursor):
        """Prompt the user to update an employee's name, salary, or department."""
        ...
        foundEmpl = DBmySQL.searchEmpl(cursor, emplDNI)    #check if employee exists
        if foundEmpl:
            match choice:
                case 1:       #update employee's name
                    newName = input("Please enter the new name for the employee: ")
                    cursor.execute("UPDATE Employee SET empName = %s WHERE dni = %s", (newName, emplDNI))
                    print("Employee name updated successfully!")
                    #show updated employee info
                    cursor.execute("SELECT * FROM Employee WHERE dni = %s", (emplDNI,))
                    updatedEmpl = cursor.fetchone()
                    DBmySQL.showEmployee(cursor, updatedEmpl)
                case 2:     #update employee's salary   
                    newSalary = float(input("Please enter the new salary for the employee: "))
                    cursor.execute("UPDATE Employee SET salary = %s WHERE dni = %s", (newSalary, emplDNI))
                ...
                case 3:      #update employee's department
                    ...
                    foundDep = DBmySQL.searchDepByName(cursor, newDep)
                    if foundDep:
                        cursor.execute("UPDATE Employee SET dprtmnt = %s WHERE dni = %s", (foundDep[0], emplDNI))
    ...
    def deleteDep(cursor):
        """Delete a department after optionally reassigning or clearing its employees."""
        depName = input("Please enter the name of the department you want to delete: ")
        foundDep = DBmySQL.searchDepByName(cursor, depName)   #check if the department exists
        if foundDep:   
            listEmplByDep = DBmySQL.showEmplByDep(cursor,depName)   #check if there are employees assigned to the department
            if listEmplByDep:
                ...
                match choice:
                    case 1:     #reassign employees to another department
                        ...
                        foundNewDep = DBmySQL.searchDepByName(cursor, newDep)      #check if the new department exists 
                        if foundNewDep:
                            cursor.execute("UPDATE Employee SET dprtmnt = %s WHERE dprtmnt = %s", (foundNewDep[0], foundDep[0]))
                            print("Employees reassigned successfully!")
                            #show employees reassigned to the new department
                            cursor.execute("SELECT e.dni, e.empName, e.salary, d.depName FROM Employee e JOIN Department d ON e.dprtmnt = d.cod_d WHERE d.depName = %s", (newDep,))
                            result = cursor.fetchall()  
                    ...
                    case 2:     #update employees' department to NULL
                        cursor.execute("UPDATE Employee SET dprtmnt = NULL WHERE dprtmnt = %s", (foundDep[0],))
                        print("Employees department set to NULL successfully!")
                    case 3:     #cancel operation
                        print("Department deletion cancelled.")
                        return
            cursor.execute("DELETE FROM Department WHERE cod_d = %s", (foundDep[0],))
            print("Department deleted successfully!") 
        else:
            print("Department not found. Please try again.")
        
    def deleteEmpl(cursor):
        """Prompt to delete an employee, handling department director reassignment if needed."""
        emplDNI = input("Please enter the DNI of the employee you want to delete: ")
        foundEmpl = DBmySQL.searchEmpl(cursor, emplDNI)         #check if employee exists
        if foundEmpl:
            cursor.execute("SELECT cod_d FROM Department WHERE depDirdni = %s", (emplDNI,))        #check if employee is a director of any department
            result = cursor.fetchone()
            if result: 
                ...
                match choice:
                    case 1:         #assign a new director to the department
                        ...
                        foundDNI = DBmySQL.searchEmpl(cursor,newDirDni)    #check if the new director exists
                        if not foundDNI:
                            input(f"No employee has been found with dni: {newDirDni}")     #assign NULL if the new director doesn't exist
                            newDirDni = None  
                        cursor.execute("UPDATE Department SET depDirdni = %s WHERE cod_d = %s", (newDirDni, result[0]))    #update the department with the new director's DNI
                        print("Department director's DNI updated successfully!")  
                    case 2:     #update the department's director DNI to NULL
                        cursor.execute("UPDATE Department SET depDirdni = NULL WHERE cod_d = %s", (result[0],))    
                        print("Department director's DNI set to NULL successfully!")
                    case 3:     #cancel operation
                        print("Employee deletion cancelled.")
                        return
            cursor.execute("DELETE FROM Employee WHERE dni = %s", (emplDNI,))  #delete the employee (as it wasn´t director or after updating the department's director)
